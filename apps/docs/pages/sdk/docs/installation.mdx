...
Step 1 and Step 2 below are interchangeable, meaning that you can run step 2 first then step 1.

### Step 1: Install `mmcv`

`mmcv` is required for `juxtapose` to perform pose estimation. Note that: `-f` i.e. `--find-links` is important here because you
will need to install `mmcv` based on your operating system and `cuda` version.
...
See detailed guide on [mmcv documentation](https://mmcv.readthedocs.io/en/latest/get_started/installation.html#install-with-pip) 
if you faced any issues while installing `mmcv`.

### Step 2: Install `juxtapose`

```bash copy filename="Terminal" /juxtapose/
pip install juxtapose
```

### Step 3: Test If The Installation Works
