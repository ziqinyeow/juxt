...
import { FabricEvent, FabricObject, KeyEvent } from "../utils";
...
class EventHandler {
  ...
  public mousewheel = (opt: FabricEvent) => {
    ...
    if (!zoomEnabled || event.e.shiftKey) {
      return;
    }
    // console.log();
    ...
    if (delta > 0) {
      zoomRatio -= this.handler.zoomStep * 2;
    } else {
      zoomRatio += this.handler.zoomStep * 2;
    }
    ...
    if (editable) {
      if (
        ...
        this.handler.prevTarget.set({
          stroke:
            this.handler.prevTarget.originStroke ||
            this.handler.prevTarget.stroke,
        });
      }
  ...
  public keydown = (e: KeyboardEvent) => {
    ...
    if (!Object.keys(keyEvent as KeyEvent).length) {
      return;
    }
    const { clipboard, grab } = keyEvent as KeyEvent;
    ...
    //   return;
    // }
    if ((e.altKey || e.metaKey) && editable && grab) {
      this.handler.interactionHandler.grab();
      return;
    }
    if (this.handler.shortcutHandler.isEscape(e)) {
